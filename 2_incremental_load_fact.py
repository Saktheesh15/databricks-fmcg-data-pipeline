...
# Check if CSV files exist in landing directory
try:
    files = dbutils.fs.ls(landing_path)
    csv_files = [f for f in files if f.name.endswith('.csv')]
    
    if not csv_files:
        print(f"No CSV files found in {landing_path}. Skipping processing.")
        dbutils.notebook.exit("SUCCESS: No new files to process")
    
    print(f"Found {len(csv_files)} CSV file(s) to process")
    
    # Proceed with reading if files exist
    ...
    if "PATH_NOT_FOUND" in str(e) or "does not exist" in str(e):
        print(f"Landing directory not found or empty: {landing_path}")
        dbutils.notebook.exit("SUCCESS: No new files to process")
    else:
        raise
